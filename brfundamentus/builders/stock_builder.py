...
def build_single_stock(info: list[str], headers: list[str], market_risk: float) -> Stock:
    """
    Build a Stock from a list of strings
    """

    map_info = {parameter.strip(): info[idx]
    ...
    average_growth = expected_growth if cagr is None or expected_growth is None else (
        expected_growth+cagr)/2

    if price_per_profit is None or average_growth is None or average_growth == 0:
        peg = None
    else:
    ...
    fair_price_graham = None if eps is None or bvps is None or eps * \
        bvps < 0 or eps < 0 or price is None or price == 0 else math.sqrt(22.5*eps*bvps)
    graham_valuation = None if fair_price_graham is None else (
        fair_price_graham/price)-1

    fair_price_bazin = None if dps is None or price is None or price == 0 else dps/0.06
    bazin_valuation = None if fair_price_bazin is None else (
        fair_price_bazin/price) - 1

    fair_price_gordon = None if cagr is None or dps is None or price is None or price == 0 else (
        1/market_risk)*dps*(1+0.1*cagr)
    gordon_valuation = None if fair_price_gordon is None else (
        fair_price_gordon/price) - 1
...
def build_list_of_stocks(csv_info: list[str], headers: list[str], market_risk: float):
    """
